...
| ![Invalid input](docs/screenshots/04-invalid-input.png) | ![Invalid sign-in](docs/screenshots/07-signin-error.png) |
| A blank name is rejected before anything is sent | Bad credentials rejected with a clear message |
...
| Invalid input failing safely | `docs/screenshots/04-invalid-input.png` (blank contact name) and `07-signin-error.png` (bad credentials), plus the 9 Postgres validation tests in [`docs/test-output.txt`](docs/test-output.txt) covering blank names, over-length names, and invalid priorities |
